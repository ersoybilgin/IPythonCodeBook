...
switches = {'pandas': True, 'numpy': True, 'html': False,
...
if switches['pandas']:
    ...
    url = "http://donnees.ville.montreal.qc.ca/dataset/f170fecc-18db-44bc-b4fe-5b0b6d2c7297/" \
          "resource/ec12447d-6b2a-45d0-b0e7-fd69c382e368/download/2013.csv"
    ...
    df[['Berri1', 'CSC', 'Mais1', 'Mais2', 'Parc', 'PierDup', 'Rachel1','Totem_Laurier']]\
        .plot(figsize=(16, 12), style=['-', '--'])
    plt.ylim(0)

    pd.rolling_mean(df[['Berri1', 'CSC', 'Mais1', 'Mais2', 'Parc','PierDup', 'Rachel1',
                        'Totem_Laurier']], 28).dropna().plot(figsize=(16, 12), style=['-', '--'])
    plt.ylim(0)
...
if switches['numpy']:
    ...
    print('\n ---- Timer 1.2, sum two arrays in NumPy: time = {0:1.3f}'.format(t2))
